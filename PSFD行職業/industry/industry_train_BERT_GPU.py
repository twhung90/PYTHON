...
# 1. 準備數據集
data_path = "D:\\Users\\survey\\Desktop\\PSFD行職業\\data\\PSFD_2018_industry_U2.csv"
dt = pd.read_csv(data_path)
texts = dt['text'].tolist()  # 文本數據
labels = dt['indust_n'].tolist()  # 標籤
...
# 創建dataloader
train_loader = DataLoader(train_dataset, batch_size=16)  # 根據需要調整批量大小
...
# 8. 保存模型
model.save_pretrained("industry_BERT_model")  # 保存模型
tokenizer.save_pretrained("industry_BERT_tokens_saved")  # 保存分詞器
np.save('industry_label_classes_BERT.npy', label_encoder.classes_)  # 保存labels類別
