...
from sklearn.utils import shuffle    #將資料隨機打散
...
class BertLSTMClassifier(PreTrainedModel):
    ...
    def save_pretrained(self, save_directory):
        ...
        }, f"{save_directory}/occupations_BERT_LSTM_model_2.pth")   #這裡會儲存LSTM的權重的檔名
    ...
    def from_custom_pretrained(cls, save_directory, *model_args, **kwargs):
        ...
        # 加载權重
        weights = torch.load(f"{save_directory}/occupations_BERT_LSTM_model_2.pth")
...
# 進行繼續訓練 (從 epoch=40 訓練到 epoch=45)
model.train()
num_epochs = 5  # 額外訓練 5 個 epoch
for epoch in range(41, 46):
    total_loss = 0
    total_correct = 0
...
model.save_pretrained("occupations_BERT_LSTM_model_2")  # 保存模型
tokenizer.save_pretrained("occupations_BERT_LSTM_tokens_saved_2")  # 保存分詞器
np.save('label_classes_BERT_LSTM_2.npy', label_encoder.classes_)  # 保存labels類別
